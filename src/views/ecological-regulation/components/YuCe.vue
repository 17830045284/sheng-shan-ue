...
const waterLevelDataObj = reactive({
...
const option1 = waterLevelPrediction(waterLevelDataObj);

const flowDataObj = reactive({
  line1Data: [2, 7, 8, 9, 10, 12, 13],
  line2Data: [8, 4, 6, 1, 2, 3, 7],
  xPointData: [
    "2025/7/20",
...
const option2 = flowPrediction(flowDataObj);

const qualityDataObj = reactive({
  line1Data: [2, 7, 8, 9, 10, 12, 13],
...
const option3 = qualityPrediction(qualityDataObj);
...
<template>
  <kt-left style="grid-template-rows: 356fr 282fr 282fr">
    ...
    <kt-item type="18" title="水位预报" :px="0">
      <div class="w-full h-full px-0 pt-10">
    ...
    <kt-item type="18" title="流量预报" :px="0">
      <div class="w-full h-full px-0 pt-10">
        <div class="w-full h-[100%]">
  ...
  <kt-right style="grid-template-rows: 244fr 320fr 358fr">
    <kt-item type="20" title="水情预报" :px="0">
      <div class="w-full h-full px-0 pt-10">
        <div
          class="w-full h-49 bg-[#001D38]/[47%] grid grid-cols-[1fr_0.8fr_1fr_1fr_1fr] items-center text-[#EAF6FF] text-[12px] SourceHanSansCNRegular"
        >
          <div class="flex justify-center">水情站</div>
          <div class="flex justify-center w-[50px]">实时水位(m)</div>
          <div class="flex justify-center w-[59px]">超(距)汛限(m)</div>
          <div class="flex justify-center w-[59px]">未来24小时水位(m)</div>
          <div class="flex justify-center w-[59px]">超(距)汛限(m)</div>
        </div>
        <Swiper
          class="deform-swiper !h-[calc(100%-50px)] !pt-[10px]"
          :modules="swiperModules"
          direction="vertical"
          :slides-per-view="3"
          :loop="hydrologicalList.length > 3"
          :speed="600"
          :autoplay="swiperAutoplay"
          ...
          :grab-cursor="true"
        >
          <SwiperSlide v-for="(row, idx) in hydrologicalList" :key="idx">
            <div
              class="w-full h-32 grid grid-cols-[1fr_0.8fr_1fr_1fr_1fr] items-center text-[#fff] text-[12px] SourceHanSansCNRegular bg-[url('@/assets/img/yuce_fiItem_bg.webp')] kt-bg-full"
            >
              <div class="flex justify-center">{{ row.name }}</div>
              <div class="flex justify-center">{{ row.curWater }}</div>
              <div class="flex justify-center">{{ row.overWater }}</div>
              <div class="flex justify-center">{{ row.futureWater }}</div>
              <div class="flex justify-center">{{ row.overDistance }}</div>
            </div>
          </SwiperSlide>
        </Swiper>
      </div>
    </kt-item>
    <kt-item type="21" title="雨情预报" :px="0">
      <div class="w-full h-full px-0 pt-10">
        <div
          class="w-full h-32 bg-[#001D38]/[47%] grid grid-cols-[1fr_0.7fr_1fr] items-center text-[#EAF6FF] text-[12px] SourceHanSansCNRegular"
        >
          <div class="flex justify-center">区域</div>
          <div class="flex justify-center">类别</div>
          <div class="flex justify-center">未来24h降雨量(mm)</div>
        </div>
        <Swiper
          class="deform-swiper !h-[calc(100%-32px)] !pt-[10px]"
          :modules="swiperModules"
          direction="vertical"
          :slides-per-view="5"
          :loop="rainList.length > 5"
          :speed="600"
          :autoplay="swiperAutoplay"
          :mousewheel="swiperMousewheel"
          :allow-touch-move="true"
          ...
          :grab-cursor="true"
        >
          <SwiperSlide v-for="(row, idx) in rainList" :key="idx">
            <div
              class="w-full h-32 grid grid-cols-[1fr_0.7fr_1fr] items-center text-[#fff] text-[12px] SourceHanSansCNRegular bg-[url('@/assets/img/yuce_seItem_bg.webp')] kt-bg-full"
            >
              <div class="flex justify-center">{{ row.name }}</div>
              <div class="flex justify-center">{{ row.type }}</div>
              <div class="flex justify-center">{{ row.value }}</div>
            </div>
          </SwiperSlide>
        </Swiper>
      </div>
    </kt-item>
    <kt-item type="19" title="水质预报" :px="0">
      <div class="w-full h-full px-0 pt-10">
        <div class="w-full h-[100%]">
